...
## 十、#45 Connector ABI + 线程安全问题（2026-07-28/29 开机修复）

### 50. CMakeCache 残留旧 VCMI_DIR
- **现象**: cmake 后 connector 链接旧版 InitArgs ABI，segfault
- **根因**: `set(VCMI_DIR ...)` 在 CMakeLists.txt 中不覆盖已缓存的变量
- **修复**: `rm CMakeCache.txt && cmake .. -DVCMI_DIR=/home/administrator/vcmi-native`
- **预防**: CMakeLists.txt 中用 `if(NOT VCMI_DIR)` 条件设置

### 51. 源码三副本不一致
- **现象**: 改完 vcmi-native 的 MLClient.cpp 后编译发现不生效
- **根因**: `vcmi-native/`（工作源码）、`vcmi-native-build/`（cmake 源目录）、`D:/Bigdata/hero3_fresh/vcmi/`（项目旧版）三份源码不同步
- **修复**: 改代码后必须 `cp` 到 vcmi-native-build（否则 make 编译的是 vcmi-native-build 下的旧代码）
- **确认方法**: `md5sum vcmi-native/ML/MLClient.cpp vcmi-native-build/ML/MLClient.cpp` 应一致

### 52. vcmi-native 工作树回退丢失 init_vcmi(void*) 实现
- **现象**: 二进制有 `T _ZN2ML9init_vcmiEPv` 但源码搜不到实现
- **根因**: 工作树 MLClient.cpp 从 690 行被回退到 616 行（旧版 3-arg init_vcmi），丢失 CreateInitArgs / init_vcmi(void*) / ML_init_vcmi
- **修复**: `cp vcmi-native-build/ML/MLClient.cpp vcmi-native/ML/MLClient.cpp`

### 53. init_vcmi 需要在主线程调用（SDL GameEngine）
- **现象**: connector 的 `start()` 在后台线程调 `ML::init_vcmi()` → segfault
- **根因**: `init_vcmi` 内部创建 `GameEngine(headless)` 初始化 SDL 子系统（即使 headless），SDL 要求主线程
- **修复**: 拆分为 `init()`（主线程）+ `start()`（后台线程只调 start_vcmi）
- **涉及文件**: threadconnector.cpp/.h, connector.cpp, strategic_env.py

### 54. boost::filesystem::create_directories 在 `./data` 上抛 EEXIST
- **现象**: init_vcmi 内 `VCMIDirs::initiatePaths()` 抛 `boost::filesystem::create_directories: File exists [system:17]: "./data", "./data"`
- **根因**: VCMI 把 userData/userCache/userLogs/userConfig/userSave 全部映射到 `./data`，5 次 `create_directories` 同路径，boost 1.83 在第二次调用时抛 EEXIST
- **当前状态**: 未解决（阻塞 #45 验证）
- **可能方案**:
  1. 修改 VCMI 运行时配置使各路径不同
  2. 在 MLClient.cpp 的 init 函数中提前 catch 该异常
  3. 改 `settings.json` 设置独立日志路径


### 55. 服务器 heredoc 写文件被 MSYS 反引号/引号绞杀（09-24）
- **现象**: `ssh root@172.16.2.40 "cat > file << 'EOF' ... "` 长脚本写入时 bash 报 "立即文档在第 1 行被文件结束符分隔" + 本地 Windows FastAPI.exe 被误执行
- **根因**: git-bash (MSYS) 对 ssh 命令串做本地展开：反引号 `` ` ``、`$`、引号嵌套在本地 shell 先解析，heredoc 结构被打碎；`$(...)` 被本地求值
- **修复**: 长文件用本地 `write_file` 写临时文件 + `scp` 推送；或 ssh 单引号包裹整个远程命令（内部避免再出现单引号）。短命令可直接双引号 ssh 但内部 `$` 要转义 `\$`
- **预防**: >20 行的远程脚本一律 scp 分发，不用 ssh heredoc

### 56. GitHub Actions runner 注册 "Must not run with sudo"（09-24）
- **现象**: root 下跑 `./config.sh` 直接拒绝：`Must not run with sudo`
- **根因**: actions-runner 明确禁止 root/sudo 运行（config 和 svc 都检查）
- **修复**: 建专用用户 `useradd -m runner` + `chown -R runner:runner /opt/actions-runner`，`su - runner -c './config.sh ...'`；svc.sh install 也由 root 跑（装 systemd unit）但 `./svc.sh install runner` 传用户名参数
- **关联**: work 目录 `/DATA/actions-work` 也要 chown runner；注册 token 从本机 `~/.config/gh/hosts.yml` 的 ghp_ token 走 GitHub API 生成（POST /repos/{owner}/{repo}/actions/runners/registration-token）

### 57. 服务器 PyTorch 是 CUDA build 无 GPU（09-24）
- **现象**: `torch.__version__` = `2.13.0+cu130`，`torch.cuda.is_available()` = False
- **根因**: 装的是 CUDA 13.0 build（sm_80/90/100/110/120 全是 NVIDIA 架构），Kunpeng ARM64 无 N 卡 → 全部回退 CPU kernel
- **影响**: 能跑（CPU 推理/评测正常），但无 GPU 加速；`torch.__config__.show()` 可确认 build 信息
- **结论**: 训练不可行（matmul ~1/1000 GPU 吞吐），推理/评测可行——见知识库 09-24 三件套定位

### 58. eval_fast.py 单进程跑 1 局 191s 且全是 DRAW/TIMEOUT（09-24）
- **现象**: 单 worker 跑 ppo_k2v_500000steps 在 Key to Victory 上 5000 步打满 191s，game_over=0
- **根因**: 该模型在此图上策略未收敛到胜利条件（5000 步 = max_turns 上限截断）；非环境 bug
- **启示**: 评测要用并行批跑出 WIN RATE 分布，单局 DRAW 不代表环境故障；批量脚本 batch_eval.py 即为此设计

### 59. 服务器 ONNX 文件是坏文件（ASCII 假 onnx）（09-24）
- **现象**: `find /DATA -name '*.onnx'` 只找到 `defender-ipnkyfqb-best3.onnx`，`file` 显示 ASCII text 非 zip
- **根因**: 内容是 `../export/defender-ipnkyfqb-best3.onnx`——某次导出把目标路径当内容写进去了（重定向方向写反或导出脚本 bug）
- **启示**: onnxruntime 加载该文件会报错但 infer service 的 try/except 会跳过；真实可用模型全在 SB3 zip。后续推 ONNX 上服务器前先 `file` 校验

### 60. 服务器 batch_eval 中途日志静默（09-24）
- **现象**: 512 局批量跑 40+ 分钟 stdout 无任何 worker 汇报行，疑似卡死
- **根因**: 脚本设计=每 worker 跑完 16 局才 as_completed 返回一条汇报；单局 3-4 分钟 → 首条汇报最早 ~50 分钟
- **判活正确姿势**: `ps aux | grep strategic_worker | wc -l`（应 33-34）+ `ps -o etime,time -p <pid>` 看 CPU time 增长 + py-spy dump 看栈（应卡 torch forward / strategic_env step）
- **教训**: 写并行脚本时汇报粒度按"单局"而非"worker 完成"；下次改 per-game as_completed
