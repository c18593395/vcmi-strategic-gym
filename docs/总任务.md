...
外部资源 (09-11 调研)：P8 联机用 vcmi/proxy-server + vcmiclient CLI 多人参数 (`--loadserver/--loadplayer`, 双客户端免 UI 直开局) + PR #4253 随机端口；P10-C 改写用 h3mtxt (h3m↔JSON) / homm3tools。详见知识库 T13/P8 章调研表。
...
2. **改代码 → `wsl -u root systemctl stop homm3-train-v5` 优雅停止 → 改完 restart (checkpoint resume)** (09-11 根修: 训练挂 system 级 enabled unit, Windows keepalive `wsl --exec sleep infinity` 防发行版容器空闲关停; 旧 user 级 transient 脚本废弃, 详见踩坑 #201 / 知识库 09-11 训练存活机制重构章)。
