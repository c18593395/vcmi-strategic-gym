...
- **09-23 打点判据假阳性面（复验补充，勿与上条结论混淆）**: `[EP298_SWALLOW]` 条件 `secs>250 且 steps<max_turns` 在**慢速轴**下同样成立（good_to_go 28 步 × ~12s/步 = 340s，steps 不满 max_turns=30）→ s600 组 good_to_go 的 `swallow=1` 是**慢速误报**，非卡死签名；要严谨区分需给打点加必要条件（该局存在 `adventure_wait timed out`）。**方法论教训（本轮实测踩到）**: `rc=124`（被外层 timeout 杀）对"卡死"与"慢速"**两条轴都成立**——s300 组 4/4 rc=124 的真因是慢速（该组 `adventure_wait timed out` 计数 = 0），**不能据此判定"S 修复无效"**；多轴现象必须拆轴取证（本次靠 s600 组 + timeout 计数才拆开，避免了"误判无效 → 错误回退有效补丁"）。
