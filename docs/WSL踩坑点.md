...
- **09-23 拆轴定谳（`py/_298_stepprofile.sh` 字节增长剖面，一击定谳）**: 每 2s 采样 ep_runner 日志字节数 → **总 332s / 最长零增长窗口 305s（27s→332s，占比 92%）/ 活跃期仅 27s 跑完 19 步 = 1.4s/步（正常）**。**结论**：① 这张图**不存在"慢速轴"**——此前"~12s/步"是"总耗时÷步数"的假象（未扣除冻结窗）；② 唯一问题是**单次 ~300s 完全冻结**（日志零增长），被内部 `adventure_wait` 300s 超时打断后收局；③ s300 组 4/4 rc=124 = 「冻结 300s + 工作 30s」> 300s 被判杀，与慢速无关；④ 修复② 对冻结**无消除作用**（修复前后均 1 次超时）。**方法论教训（本轮踩到，两次误判）**: **判断"卡死 vs 慢速"必须用增长剖面，不能用总耗时÷步数**——同一组数字支持两种相反解释，只有剖面能分开；且 `rc=124` 对两者都成立，不能单独作判据。
