...
#### #302 `git merge-tree --name-only` 输出不是纯名单：夹 info 消息 + submodule trailer → 按总行数当冲突数会高估 8~25 倍 (09-24 自伤实证) — ✅ 已纠正

- **状态**: ✅ 已纠正（口径改为数 `CONFLICT` 行 / 名单段条目数），台账 §1.1 已按正确值重写
- **现象**: 复算分叉面时把 `git merge-tree --write-tree --name-only <分支> <官方ref>` 的**输出总行数减 1** 当成冲突文件数，得到 `fix_action_mapping` = 193、`mmai-ml-wsl` = 2543；据此还**错判台账原记的「32 个冲突」复现不出来**，并一度准备把错误值写进台账。
- **根因**: `--name-only` 只裁剪「冲突条目」那一节，输出实际是三段拼接：① 第 1 行 = merged tree OID；② **名单段**（真正的冲突文件，`--name-only` 时一行一个）；③ **信息性消息段**（`Auto-merging <file>` ×117、`CONFLICT (content): Merge conflict in <file>`、以及 `dependencies` 子模块的 "come back to superproject / git add dependencies" 多行 trailer）。第 ③ 段行数远大于第 ② 段 → 总行数严重高估。
- **正确口径**: `grep -c '^CONFLICT'`（按类型可再拆分：`grep '^CONFLICT' | sed 's/:.*//' | sort | uniq -c`）；或取名单段条目数（第 2 行至**首个空行**前，用 `awk 'NR>1 && NF==0 {print NR-2; exit}'`）。两者一致：fam = **32**（content 30 / submodule 1 / modify-delete 1），wsl = **786**（content 393 / add-add 370 / modify-delete 11 / rename-rename 5 / rename-delete 4 / file-location 2 / submodule 1）。
- **教训**: ① 解析 git 机器可读输出**必须先确认段结构**，不能默认"每行一条记录"——`--name-only` 名字很像纯名单但不是；② 用某个数字**去否定既有结论前**，先拿一个已知答案的样本对拍（本次若先对 `fix_action_mapping` 验一次就能立刻发现 193≠32）；③ 「数错」与「原文错」的结论强度完全不同——差一步就把正确数字改错，**报"复现不出来"需附一次同口径的正向验证**。
- **关联**: #303（同一次取证中的姊妹误判）/ 台账 `docs/官方同步台账.md` §1.1（口径脚注）/ 对照样本：`fix_action_mapping` = 32 与台账原记一致

#### #303 跨仓"漂移"取证必须先对齐分支：拿 A 仓工作树 vs B 仓工作树 diff，会把分支演进误判成"拷贝漏同步" (09-24 自伤实证) — ✅ 已纠正

- **状态**: ✅ 已纠正（撤回定性，台账 §1.1「已撤回的误判」留档）；属 `py/audit_r5_trees.sh` 的**设计前提**，该脚本默认 "A 与 B 应语义一致"，同分支才成立
- **现象**: 用 `py/audit_r5_trees.sh` 及 `diff --strip-trailing-cr` 比 `D:\Bigdata\hero3_fresh\vcmi`（分支 `fix_action_mapping` @1c3be8d03）与 `/home/administrator/vcmi-native`（分支 `mmai-ml-wsl` @8679dd35a1），得 7 文件差异（CGameHandler 29 / strategic_state.cpp 31 / Client.cpp 21 / AIGateway 6 / CServerHandler 1 / strategic_state.h 1 / CMakeLists 1 行），据此下了"两棵树拷贝漏同步 / WSL 侧有未回流热修"的结论。
- **根因**: **两边不同分支**。这 7 处差异主体是 `mmai-ml-wsl` 自 08-29 以来的正常分支演进（`bothAI` 短路、`s_force_game_over`、`onNetworkThread`、passable 闸门修正等本就是该分支的提交内容），不是"同一份代码被改坏"。**先设了"两棵树"的错误心智模型，才去找能印证它的差异。**
- **附带教训（工具层，复发 #154）**: 期间用 `wsl bash -c 'for d in ...; do echo $d; done'` 判"`vcmi-native` 无 `.git`"，因 **PowerShell 层把 `$d` 吃掉**（单引号也拦不住）导致循环体内变量为空、**静默失败**，于是误判目录不是 git 仓，进而把拓扑描述错。→ 与 #154 同源，**复发**。正确做法：避免在 `wsl bash -c` 串里用 `$var`/`$( )`，或写成脚本文件再执行；**判断"某目录没有 X"前要用不依赖变量的命令复验一次**。
- **教训**: ① 比差异前先 `git -C <repo> rev-parse --abbrev-ref HEAD` / `--short HEAD` 确认两侧**同分支同源**，不同分支的 diff 不构成"漂移"证据；② 相似脚本（`audit_r5_trees.sh`）的隐含前提要写进脚本头部，否则后人照跑照误判；③ "发现差异"到"定性为故障"之间必须过一道**归因替代解释**（分支不同？CRLF？生成物？）的检查。
- **关联**: #302（同一次取证的姊妹误判）/ #154（PowerShell 吞变量，本条复发）/ `py/audit_r5_trees.sh` / 台账 §1.1 / 真正的同分支对比结果见 #302 正确口径

#### #304 中转裸镜像兼灾备源：删除前必须验 ref 全覆盖，且要连带修 `setup_wsl_train.sh` 的 fallback，否则灾备静默回退 (09-24 实锤) — ✅ 已处理

- **状态**: ✅ 已处理（镜像已删 395.6 MB；10 个未推送提交先保全；`setup_wsl_train.sh` 灾备源已改）
- **现象**: 用户要求删除中转镜像 `D:\Bigdata\git-mirrors\vcmi-native.git`。核查发现它不是单纯中转，而是**同时承担三个角色**：① 仓#2（WSL 真实编译树）的 `origin`；② `py/setup_wsl_train.sh` 步骤 4 的**唯一克隆源**（WSL rootfs 重建用）；③ 仓#2 当时 **10 个未推送提交的唯一远端副本**。
- **根因/风险链**: 直接删 → `origin` 悬空 + 仓#2 的 10 提交（含 #298 恢复、09-24 五处修复）**无任何远端副本**。更隐蔽的是 `setup_wsl_train.sh` 的 fallback：`cp -r $PROJ/vcmi` 拷的是**仓#3 的 `fix_action_mapping` 工作树**（无 `.git`、且不是 `mmai-ml-wsl`）→ **WSL 重建后静默回退到 09-19 前的代码**，脚本不报错、日志不提示，属于"灾难恢复反而制造灾难"。
- **处理（正确顺序）**: ① **先保全**：仓#2 → 仓#3 快进推送 `mmai-ml-wsl`（`30f62b867a..8679dd35a1`，10 提交/1202 文件，`--dry-run` 预检后执行）；② **再验无唯一对象**：镜像三分支 `1c3be8d030`/`d6f8f373a8`/`20ba315945` 逐一 `git cat-file -t` 确认仓#3 都有，且 `d6f8f373a8` 已是 `mmai-ml-wsl` 祖先（`merge-base --is-ancestor`）；③ 才删除；④ **善后**：仓#2 `origin` 改指 `/mnt/d/Bigdata/hero3_fresh/vcmi`（`git ls-remote` 复验可见）+ `setup_wsl_train.sh` 的 `MIRROR=` → `SRC=$PROJ/vcmi` 并**删掉 `cp -r` fallback**（源缺失改 `FATAL` 退出）。
...
- **现象**: 核查训练状态时对照三处 MIX 值，**三个都不一样**：
  ...
  - `train_loop.log` 最后一次实跑的 `[MIX_TRACE]` → `mix=0.2`
- **根因（推测）**: unit 双副本靠**手工同步**，历次改 MIX（0.10 → 0.2 → 0.5 的观测窗演进）只改了其中一处；且 `/etc` 现值 0.50 **晚于**最后一次实际运行（服务当前 inactive，下次 start 才会取 0.50）。
- **影响**: ① 读 `py/` 副本会以为在跑 0.10，读 `/etc` 会以为 0.50，**两者都可能不是上一窗真正生效的值**；② MIX 直接决定池图出现率，误判会让"池图零出现是否异常"的判据全错（#300 已因样本不足+归属错误吃过一次这个亏）。
- **教训**: ① 配置判据只认**运行时实际值**（日志打点 / 进程 environ / `systemctl show -p Environment`），**副本文件与文档一律不作准**（与项目"事实核查原则"一致）；② 双副本（`/etc` + `py/`）必须带一条同步校验，或干脆单一来源；③ 任何"改 MIX/开新轴"的操作记录必须写明**改在哪一处**，否则下次无从复盘。
- **关联**: #300（池图出现率判据，同域）/ 项目规则「训练进程运维」/ `py/homm3-train-v5.service` / `train_wsl2_ppo_v2.py` L537-543（`[MIX_TRACE]` 打点）
