...
### #332. 任务清单文档熵增三根因——置顶纪律混入 / 已归档内容不删 / 编号体系泛滥 ✅ 治理固化（09-27）

- **现象**：当前任务清单.md 从"当下窗口要做什么"膨胀至 420+ 行，约 60% 为已归档历史重复（"最近整理"六段 + 09-23/09-24 收口原文 + 各 ✅ 任务详情）；顶部混入三条本属公共规范的置顶纪律（改动流向 / keepalive / 竞态验证）；编号体系并存 T.n / WIN-x / P0-P3 / S-x / A-x / OBS-x / R-x 七套，且优先级编死在编号里（P0-1 降级就得改名断引用）。
- **根因**：① 无公共规范载体 → 纪律性内容全堆清单顶部；② "完成即移出"原则在但无强制动作 → 收口段落写完清单不删归档文档也不承接，两边都长；③ 🟡"部分完成"态无收口机制 → 混合状态条目长期挂活跃区；④ 每类新工作开新编号前缀 → 引用面碎。
- **治理（09-27 用户拍板，固化为 `docs/项目管理.md` §二）**：① 公共规范独立项目管理.md 唯一权威，project_rules.md 只留指针；② 编号收敛 T + P 流水号（优先级是字段），部分完成自动拆子号（T14.4A/B/C 式）父任务必须收口；③ 状态符号收敛四种 ✅🔄⬜❌（🟡🔶⏹ 废除，"待触发"= ⬜ + 触发条件写入描述）；④ 完成即删条，清单只留销项登记，详情只在已完成任务.md。
