...
- **09-23 修复② 实测失败（heroExchange bothAI 自动关闭）**: 补丁 `py/patch_298_heroexchange.py`（`CGameHandler::heroExchange` 锚点前插 bothAI 短路 → `useScholarSkill()` + return，对齐 `makeGarrisonDialog` 08-27 模式，代价 = AI 间军队/宝物合并放弃；备份 `.bak_298_0923`）→ **03:09 libvcmi.so 重编完成** → **03:11 起验证（跑在重编之后，结果有效）** → `good_to_go` / `judgement_day` **双双 rc=124（被外层 timeout 300 强杀）**，elbow/viking 进行中。**判据**：修好应 ~40s rc=0；rc=124 = 仍在 300s 内部强停窗口 → **未修好**。**教训**：① 补丁式修复必须"**改-编-验**"三段留痕——本次三段齐全，所以 rc=124 能直接归因"补丁无效"而非"没编进去"（缺任一段则结论不可用）；② **验证脚本 timeout 必须显著大于被测现象的等待窗口**——300 vs 300 同量级 → 判据退化成二分，且强杀使**局尾打点永不触发**（观测数据与结论一起丢失）；③ **重编 libvcmi.so 属铁律例外，必须当场登记**（本次已登记待拍板）。**盲区假设**：只堵 heroExchange 一条通道，而定谳栈顶是 `CGarrisonDialogQuery qid=23` → Garrison/town 路径可能未被覆盖，待归因（`py/_298_grep_garrison.sh`）。
