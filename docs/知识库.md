...
### 3.6 step() 通信机制（C7 最终架构 + B 修复）

```
Python step(action):
  ① _send_action(action)        // 发 action 给 VCMI（写 atomic s_turn_action）
  ② _adventure_wait()           // 等 VCMI 处理完在下一轮 process_turn 阻塞
  ③ _read_state()               // 读状态（战报+passability已更新）

C++ yourTurn() → 单次 action:
  ① adventure_process_turn()    // 阻塞等 Python 发 action（spin-loop on s_turn_action_ready）
  ...
  ④ selectionMade(queryID)      // 回答 query（moveHero 之后再答，防"Player not active"拒绝）
  ⑤ strategic_state_update()    // 填充 StrategicState（CGameState 可能为空，见 #45）
  ⑥ compute_passability_cb()    // 用 cb->canMoveBetween() 算 8 方向可通行性（非 brute-force all-1）
  ⑦ cb->endTurn()
```

**关键变更（B 修复 2026-07-28）：**
1. moveHero 在 selectionMade 之前执行 → 不被服务器拒绝
2. passability 用 `cb->canMoveBetween()` 替代 `passable[d]=1` 暴力覆盖
3. fallback 路径也用 `adventure_get_action()` 而非 `g_rl_action`
