...
### 高负局解剖与 #292 二次修复（09-21 停训窗，T7.8 观测窗首轮数据分析）

**观测窗 120 局全景**（恢复后窗口，`py/_neg_pattern_0921.py` 从主日志 ep_steps 行提取 act 序列分析）：

```
rew: min=-808  median=-71  mean=-108.6  max=196
高负(<-300): 14 局 (11.7%)  /  正局: 31%
训练器 12 局窗 avg_r: -3.0~-0.7 (历史满窗 -77~-144 → 量级改善, 绕怪豁免+nk2 修复生效)
```

**高负局全部同构**（8 局 -557~-808 一个模子）：T06 非 duel（1v3 多敌）+ King_of_Pain；250 步满步慢性失血（非速死）；act **[3,7] 对角对拍**（周期覆盖率 283%）；**END_TURN=0**（250 步一次没有）；同 scorer pick 连续 50 拍（`[SCORE] pick=(16,24) plen=1` 重复）。数字对账：-3/拍 × 250 = -750 ≈ -740 ✓。

**根因三连**：① #292 首版漏写递增行（计数器恒 0）；② zigzag 段挂 `a<8` 分支——MOVE_TO(24) 展开的横跳整段不执行（引导路径盲区）；③ move_stall "距离减小即清零"——[3,7] 增/减交替永不达 6；scorer 无单目标拉黑。

**修复四件**（主仓 8303ca2）→ **三次修复终版**（c61e4db）：首版二次修复实战 -909 局仍循环——真实横跳形态 = act3 连发 + 多格晃（非严格 [3,7] 对拍），prev2 判定与距离启发式都漏。**终版换锚点：`ts_pick_count` 同目标累计 pick ≥8 → `[SCORE_BLACK]` 拉黑**——不依赖位置启发式，直接锚定"同目标长时间完不成"。冒烟：step28 [GUIDE_OSC_DROP] + 3 死格逐拉黑 → 190 步正常收局 r=-393。配 [GUIDE_OSC_DROP]（≥6 放弃+拉黑）+ `ts_blocked` 单目标拉黑（城目标豁免防自毁占城）+ zigzag 计数每拍更新（奖励面零变更）。

**方法论沉淀**：
- **act 序列周期检测**是定位死循环的最快手段：ep_steps 行自带完整 act 序列，穷举周期 2-12 统计覆盖率，283% 覆盖 = 严格对拍实锤（工具 `_neg_pattern_0921.py` 常驻 py/）
- **熔断器三查**：上线后不触发时按序查 ① 递增行存在？② 外层分支可达（动作类型/路径）？③ 消费点在正确执行路径上？
- **奖励面与行为护栏分离**：修护栏（熔断/拉黑）不动奖励口径，观测窗数据不被塑形变更污染
- 高负局画像变化：修完横跳后剩余高负 = 1v3 能力性战死（打不过三个），图池混合轴上线后自然稀释

关联：踩坑 #292（二次修复全文）/ `py/_neg_pattern_0921.py` / 主仓 `8303ca2`

### 终版护栏新窗首批信号（09-22 晚，ep42 窗 14 局——duel 首批转正）

MIX=0 纯课程图 + 终版护栏（#292 三次修复 + ts_pick_count 拉黑）运行 2.5 小时（ep42）：

- **duel 首批转正**：108_02_duel **+28.6** / 72_01_duel **+5.1**（正局 ×2，历史 duel 全负居多）；72_02_duel -43~-48 小负
- **零高负**：14 局最差 -180.1，无 -300+；护栏打点（SCORE_BLACK/GUIDE_OSC_DROP）零复发
- **King_of_Pain 持续改善**：-85.3 / -180.1（修复前 -311~-430）
- 14 局 r 均值 **-60.0**（上窗 -68~-86）；avg_r -0.2~-0.8 健康

**判断**：与回滚对照（当前 ckpt 好 27%）互相印证——训练正优化，duel 主战场率先突破。样本尚小，攒满 100 局跑正式四判据；按轨迹判据②有望首次接近达标。
...
ERROR Can not end turn for player that is not in game!
ERROR Got false in applying 7EndTurn... that request must have been fishy!
（循环刷屏 → adventure_wait timed out after 300s）
...
- **判定（拆轴定谳：09-23 04:06 `py/_298_stepprofile.sh` 字节增长剖面实测）**: ① **不存在"慢速轴"——"~12s/步"是误读**（用"总耗时÷步数"算出来的假象，未扣除冻结窗）。剖面实测：总 332s，**最长零增长窗口 305s（27s→332s，占比 92%）**，活跃期仅 27s 跑完 19 步 = **1.4s/步（与课程图 1.1s/步同量级，正常）**；② **卡死轴 = 唯一问题且仍在**：单次 ~300s 完全冻结（日志零增长），被内部 `adventure_wait` 300s 超时打断后收局；③ 因此 s300 组 4/4 rc=124 = 「~300s 冻结 + ~30s 正常工作」> 300s 被判杀，**与慢速无关**；s600 组 rc=0 同理（冻结 300s + 工作 40s = 340s）；④ 修复② 对冻结**无消除作用**（修复前后均 1 次超时），"吞局→收局"的差异单局样本不足以归因；⑤ judgement_day 另有 **boot hang 2/2**（卡 TERRAIN 段、adventure 未进）→ 与 patch 无关，单独定位。
- **打点判据复核（09-23 剖面后修正）**: 既然步进速度正常（~1.4s/步），`[EP298_SWALLOW]` 的 `secs>250 且 steps<max_turns` 基本等价于"存在冻结"（30 步预算下正常不可能超 250s）→ **判据成立**，此前记的"慢速假阳性面"作废。窄例外：250 步满步预算下若提前战死（如 200 步/280s）会误报，可用"该局存在 `adventure_wait timed out`"作必要条件加固。
- **语义代价评估（去留待定）**: 补丁取消 AI 间军队/宝物合并 → 训练语义有变，而**对冻结的消除作用未被证明**（修复前后均 1 次超时，单局样本不足）。并行会话主张保留（课程图 9000+ 局 exchange 从未卡死、影响小），我倾向"再取 3-5 局配对样本再定去留"——**当前不动**（既不回退也不追加）。
...
- **漂移取证（本窗口实况核查）**: 漂移**远超初判**——WSL 构建树 `~/vcmi-native` 工作区相对 HEAD：**3028 文件 / 79959 insertions / 73473 deletions / 5074 hunk**（另有 1541 文件仅模式变更 644→755）。含整个 `ML/` 目录（`ML/MLClient.cpp` 610/610）、`AI/MMAI/BAI/fallback/MLBot.cpp` 639/56、`AI/Nullkiller2/Pathfinding/AINodeStorage.cpp` 641/233、`AI/Nullkiller2/Helpers/ExplorationHelper.cpp` 546/43、`lib/battle/CBattleInfoCallback.cpp` 271/91、`client/render/AssetGenerator.cpp` 668/1 等 → **这是一次完整的「上游重同步 + ML fork 代码」，不是"500 行小改动"**。
- **⚠ 勘误（本窗口自纠，重要）**: 先前记的"6 文件 / 508+/214-"是**误报**——那次 `git diff --stat` 带了 6 个文件的 pathspec，只统计了子集。真实漂移见上条。→ **当前跑训练的 .so 与 HEAD 严重分叉，无法从已提交源码复现**。
- **入库策略（advisor 复核后定，勿全树 `git add -A`）**: 只入库**已分类的 6 个文件**（`CGameHandler`/`CServerHandler`/`Client`/`QueriesProcessor`/`AIGateway`/`BattleResultProcessor`），commit message 显式标注两处 08-17 回退为「**已知回退，有意保留待恢复**」；剩余 ~3000 文件（尤其 `ML/MLClient.cpp`、`MLBot.cpp`、`AINodeStorage.cpp`）**另起一轮专项盘点**，不与这 6 文件混提（防未分析代码被洗进历史）。
