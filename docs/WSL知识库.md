...
- 新建 `docs/项目管理.md` = 公共规范唯一权威（文档权威侧 / 编号规则 / 长期纪律 / 开关机 / 训练启停 / git / 多 agent 协作 SOP）；project_rules.md 精简为最小硬约束 + 指针，防双权威漂移（#306 同型教训）。
- 编号收敛：T 系列（大项）+ P 流水号（P-001 起，优先级 P0-P3 是字段不是编号）；WIN 编号废除新设；部分完成自动拆子号（T14.4A/B/C 式，父任务必须收口）。
- 状态符号收敛四种：✅ 已完成 / 🔄 进行中（含观察中、部分子项完成的大项）/ ⬜ 未开始（含等触发，触发条件写入描述）/ ❌ 取消终止；🟡🔶⏹ 废除。
- 清单最终结构：一、文档关系 → 二、P 系列当前任务（28 项）→ 三、T 系列任务（✅ 历史 + 🔄 活跃 + ⬜ 远期全带 T 编号）→ 四、观察与风险。

**二、T5.9 / T6.3 是什么（通俗定谳，清单入口归 T6.3 一条）**：
- T5.9 = 动作从扁平码改三维组合（动作类型, 目标对象, 参数），治码组合爆炸与低频码探不到（09-25 方案 B 实锤：13/14/15 在 148 份 traj 零选中，靠强制采样窗旁路）。
- T6.3 = 决策分层（上层选目标、下层生成动作序列），T5.9 参数化是其技术前提，合并为一个工程。

**三、T5.9/T6.3 + T12 时机分析（09-27 用户问"现在做是否更好"，结论 = 均不立即做）**：
- 重训影响面：obs 3464 冻结 → obs 编码器权重可 warm-start；动作头重初始化；旧 traj 全作废；BC 数据看"旧码→参数"兼容层，否则重采集；C++ AAI 执行分支 + runner mask + trainer 全改。工程估 1-2 周工程 + 1-2 周验证。
- T5.9/T6.3 现在做的代价：在途判据全作废（batch3/duel/T7.8/能力门全部重基线化，错窗纪律下主线停 2-4 周）；瓶颈未实锤在动作表示（当前失分点 = 冻结/图池/战死，低频码问题已被强制窗低成本缓解且 RECRUITED 已达标）；与 T12 重复建设（两次迁移最差）。
- **T12 量级**：全链路推倒（图状态取代 3464 向量 + GNN 取代 MLP + py::dict 取代 struct + 训练体系重写），数周级工程 + 数周级重训；参照 T14.3 单模块 2-3 天、T5 冻结 2 天，T12 是数倍。
- **T12 三条硬伤（现在做）**：① 上游流沙 = v15 是 MMAI PR #7654 未合并官方 + 2-4x 慢，基于未定型上游全量迁移，上游一改再来一次；② 吞吐红利被吃（刚迁服务器 ×8-16，架构慢 2-4x 吐回一半）；③ 当前瓶颈（冻结/图池/战死）与表达力无关，且 A3 闸门/撤梯子等调熟经验全绑旧体系，迁移后重调；新架构初期性能大概率低于调熟 MLP 基线；连对照基线都不存在（obs 都变了）。
- **定谳路线 = 备料 + 触发条件，两档合一**：
  - T6.3（含 T5.9）触发：强制采样窗撤除后低频码频率塌回 0，或 T7.8 收口后 avg_r 长期滞涨 → 才立项实施；此前只做设计稿备料（映射表/迁移步骤/回退线，T14.2 模式）。
  - T12 触发（三条件同时满足）：① v15 合并官方（上游定型，硬前置）② 旧体系能力验证到平台期（T7.8 收口 + batch 体系稳 + T14.4C 三档对手谱出炉留对照）③ Phase G 节点。
  - 备料项（零风险可先行）：T12.1 图状态设计稿（基于 3464 obs 语义段拆节点/边）；低成本跟踪 PR #7654 合并状态。
- 一句话：**T12 是终局必经路，但"未合并 v15 + 2-4x 慢"决定现在做是把地基打在流沙上；T5.9/T6.3 是它的子集问题，一并挂触发条件。**
