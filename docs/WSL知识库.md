...
**修复四件**（主仓 8303ca2）：递增行补全 + 计数器每拍基于位置更新（zigzag -2.0 扣分维持 a<8 原口径，**奖励面零变更只修护栏**）+ 引导路径消费 `[GUIDE_OSC_DROP]`（≥6 放弃+拉黑）+ `ts_blocked` 单目标拉黑（城目标豁免防自毁占城）。冒烟：同图 72_02 修复前 -803（250 步循环）→ step28 熔断、120 步战死正常收局。
