...
class VAE(nn.Module):
    def __init__(self, input_dim, layer_sizes):
        ...
        # No activations at the encode-decoder boundary
        encoder.pop(-1)
        decoder.pop(-1)
        ...
        self.fc_mu = nn.Linear(encoder[-1].out_features, encoder[-1].out_features)
        self.fc_logvar = nn.Linear(encoder[-1].out_features, encoder[-1].out_features)
...
def eval_vae(logger, vae, buffer, eval_env_steps):
    vae.eval()
    ...
    for batch in buffer.sample_iter(batch_size):
        reconstructed, mu, logvar = vae(batch)
        losses.append(vae_loss(reconstructed, batch, mu, logvar).item())

    return sum(losses) / len(losses)


def train(resume_config):
    run_id = "vae-" + ''.join(random.choices(string.ascii_lowercase, k=8))
    ...
    obs = to_tensor(dict_obs)
    vae = VAE(input_dim=obs.shape[0], layer_sizes=config["train"]["layer_sizes"])
