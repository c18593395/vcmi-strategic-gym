...
def compute_loss(action, logits_main, logits_hex):
    ...
    # (B) of actions
    actions_on_hex = torch.where(target_main == MainAction.HEX)[0]
    # => (B') of indexes
    ...
    # => split:
    #   logits_hex_score  (B', 165)
    #   logits_hex_action (B', N_HEX_ACTIONS)
    #
    ...
    # 1) Compute log-probs for each head
    logp_main = F.log_softmax(logits_main, dim=1)        # (B, 3)
    logp_hex = F.log_softmax(logits_hex_score, dim=1)    # (B', 165)
    logp_ha = F.log_softmax(logits_hex_action, dim=1)    # (B', 14)

    # 2) Initialize joint log-prob with main head for all samples
    target_main_unsq = target_main.unsqueeze(1)                         # (B, 1)
    joint_logp = logp_main.gather(1, target_main_unsq).squeeze(1)       # (B,)

    # 3) Add hex and hex-action contributions only for HEX actions
    is_hex = (target_main == MainAction.HEX)                             # (B,)
    if is_hex.any():
        # Map batch indices from full B to B' for hex heads
        idx_full = torch.nonzero(is_hex, as_tuple=False).squeeze(1)      # (B',)
        # Gather hex-location log-probs
        hex_ids = target_hex_id                                          # (B',)
        loc_logp = logp_hex.gather(1, hex_ids.unsqueeze(1)).squeeze(1)   # (B',)
        # Gather hex-action log-probs
        ha_ids = target_hex_action                                       # (B',)
        action_logp = logp_ha.gather(1, ha_ids.unsqueeze(1)).squeeze(1)  # (B',)
        # Accumulate into joint_logp
        joint_logp[idx_full] += loc_logp + action_logp

    # 4) Compute final negative log-likelihood loss
    ...
    for epoch in range(epochs):
        ...
        for batch in buffer.sample_iter(batch_size):
            ...
            with maybe_autocast:
                ...
                loss_tot = compute_loss(action, pred_logits_main, pred_logits_hex)
