...
def vcmi_dataloader_functor():
    def mw(data: Data, ctx: Context):
        ...
        if ctx.transition_id == 0 or data.done or (ctx.transition_id == ctx.num_transitions - 1):
...
def compute_loss(obs, action, logits_main, logits_hex):
    assert all(action > 0), "Found invalid actions: %s" % action


    # WIP: target action is 0..2312, but I will not predict 0 (RETREAT)
    # as it's pure noise
    # => predicts 0..2311 corresponding to targets 1..2312
    ...
    gmask_start = GLOBAL_ATTR_MAP["ACTION_MASK"][1]
    gmask_end = GLOBAL_ATTR_MAP["ACTION_MASK"][2] + gmask_start
    invalid_gmask = ~(obs_global[:, range(gmask_start, gmask_end)].bool())
    # => (B, 2) bools where True=invalid, False=valid action

    hexmask_start = HEX_ATTR_MAP["ACTION_MASK"][1]  # offset
    hexmask_end = HEX_ATTR_MAP["ACTION_MASK"][2] + hexmask_start  # n + offset
    obs_hexes = obs_hexes.unflatten(-1, [165, STATE_SIZE_ONE_HEX])
    invalid_hexmask = ~(obs_hexes[:, :, range(hexmask_start, hexmask_end)].bool())
    # => (B, 165, 14) bools where True=invalid, False=valid action
    ...
    for epoch in range(epochs):
        ...
        for batch in buffer.sample_iter(batch_size):
            ...
            with maybe_autocast:
                ...
                loss_main, loss_hex, loss_hexaction = compute_loss(obs, action, pred_logits_main, pred_logits_hex)
