...
class TransitionModel(nn.Module):
    ...
    def predict_from_probs_(self, obs, action_probs, strategy=Reconstruction.GREEDY):
        logits_obs = self.forward_probs(obs, action_probs)
        return self.reconstruct(logits_obs, strategy=strategy)

    def predict_(self, obs, action, strategy=Reconstruction.GREEDY):
        logits_obs = self.forward(obs, action)
        return self.reconstruct(logits_obs, strategy=strategy)
    ...
    def predict(self, obs, action, strategy=Reconstruction.GREEDY):
        with torch.no_grad():
            ...
            obs_pred = self.predict_(obs, action, strategy=strategy)
            return obs_pred[0].numpy()
