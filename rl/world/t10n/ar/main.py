...
def do_test(model, env):
    ...
    for _ in range(10):
        ...
        for i in range(1, len(obs["transitions"]["observations"])):
            ...
            total_loss, losses = ar.compute_losses(
                logger=None,
...
if __name__ == "__main__":
    ...
    if args.action == "test":
        ...
        common_args = dict(
            ...
            # sample_only=False,
            model_creator=ar.ARTransitionModel,
            weights_builder=build_feature_weights,
            buffer_creator=ar.Buffer,
            vcmi_dataloader_functor=ar.vcmi_dataloader_functor,
            s3_dataloader_functor=None,
            eval_model_fn=ar.eval_model,
            train_model_fn=ar.train_model
        )
