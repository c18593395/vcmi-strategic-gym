...
class AgentNN(nn.Module):
    ...
    def __init__(self, network, input_dim):
        super().__init__()

        with open(network.autoencoder_config_file, "r") as f:
            autoencoder_config = json.load(f)

        self.encoder = Autoencoder(input_dim=input_dim, layer_sizes=autoencoder_config["train"]["layer_sizes"]).encoder
        ...
        self.actor_head1 = AgentNN.build_layer(network.actor_head1)
        self.actor_head2 = AgentNN.build_layer(network.actor_head2)
        self.critic = AgentNN.build_layer(network.critic)

        # Init lazy layers
        with torch.no_grad():
            bodyout = self.body(torch.randn(self.encoder[-1].out_features))
            self.actor_head1(bodyout)
            self.actor_head2(torch.randn(bodyout.shape[0] + 1))
            self.critic(bodyout)

        common.layer_init(self.body)
        common.layer_init(self.actor_head1, gain=0.01)
        common.layer_init(self.actor_head2, gain=0.01)
    ...
    def get_action_and_value(self, b_obs, b_mask1, b_mask2, b_action1=None, b_action2=None, attn_mask=None, deterministic=False):
        ...
        b = b_obs.shape[0]
        b_features = self.body(self.encoder(b_obs))
...
class Agent(nn.Module):
    ...
    def __init__(self, args, observation_space, action_space, state=None, device="cpu"):
        ...
        self.NN = AgentNN(args.network, observation_space["observation"].shape[0])
        self.NN.to(device)
        self.optimizer = torch.optim.AdamW(filter(lambda p: p.requires_grad, self.NN.parameters()), eps=1e-5)
...
class JitAgent(nn.Module):
    ...
    def get_value(self, obs) -> float:
        with torch.no_grad():
            b_obs = obs.unsqueeze(dim=0)
            features = self.body(self.encoder(b_obs))
...
def debug_args():
    return Args(
        ...
        network=dict(
            ...
            body=[
                dict(t="LazyLinear", out_features=128),
                dict(t="LeakyReLU"),
                dict(t="LazyLinear", out_features=128),
                dict(t="LeakyReLU"),
            ],
            actor_head1=dict(t="LazyLinear", out_features=13),
            actor_head2=dict(t="LazyLinear", out_features=165),
