...
#  9 => [5, 6, 8, 9, 0, 0, 0]
#
# Input: (B, ...) reshapeable to (B, 11, 15, hex_size)
# Output: (B, 165, hex_size)
#
class HexConv(nn.Module):
    def __init__(self, hex_size):
        ...
        self.fc = nn.Linear(in_features=hex_size*len(padded_offsets0), out_features=hex_size)
...
class HexConvResLayer(nn.Module):
    def __init__(self, hex_size):
        ...
        nn.init.kaiming_uniform_(self.body[0].fc.weight, nonlinearity='leaky_relu', a=self.leaky_relu.negative_slope)
        nn.init.zeros_(self.body[0].fc.bias)

        nn.init.xavier_uniform_(self.body[2].fc.weight)
        nn.init.zeros_(self.body[2].fc.bias)

    def forward(self, x):
        return self.leaky_relu(self.body(x).add(x))


class HexConvResBlock(nn.Module):
    def __init__(self, hex_size, depth=1):
        super().__init__()

        self.layers = nn.Sequential()
        for _ in range(depth):
            self.layers.append(HexConvResLayer(hex_size))
...
class Model(nn.Module):
    ...
    def __init__(self, config):
        ...
        kaiming_init(self.encoder_other.linear)
        kaiming_init(self.encoder_hexes.linear)
        kaiming_init(self.encoder_merged.linear)
    ...
    def get_actdata_eval(self, obs):
        ...
        return self._get_actdata_eval(z_merged, obs)

    def get_actdata_train(self, obs, action):
        z_merged = self.encode(obs)
        return self._get_actdata_train(z_merged, obs, action)
...
def collect_samples(logger, model, venv, num_vsteps, storage):
    ...
    for vstep in range(0, num_vsteps):
        ...
        actdata = model.model_policy.get_actdata_eval(storage.next_obs)
        value = model.model_value.get_value(storage.next_obs)
    ...
    next_value = model.model_value.get_value(storage.next_obs).reshape(1, -1)
...
def eval_model(logger, model, venv, num_vsteps):
    ...
    for vstep in range(0, num_vsteps):
        ...
        obs = t(obs)
        actdata = model.model_policy.get_actdata_eval(obs)
...
def train_model(
    ...
    for epoch in range(train_config["update_epochs"]):
        ...
        for i, start in enumerate(range(0, batch_size, minibatch_size)):
            ...
            new_z = model.model_policy.encode(mb_obs)
            new_actdata = model.model_policy._get_actdata_train(new_z, mb_obs, old_actdata.action)
