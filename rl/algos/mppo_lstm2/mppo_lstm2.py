...
@dataclass
class NetworkArgs:
    features_extractor_stacks: list[dict] = field(default_factory=list)
    features_extractor_hexes: list[dict] = field(default_factory=list)
...
class AgentNN(nn.Module):
    ...
    def __init__(self, network, action_space, observation_space):
        ...
        self.features_extractor_stacks = torch.nn.Sequential()
        for spec in network.features_extractor_stacks:
            layer = AgentNN.build_layer(spec)
            self.features_extractor_stacks.append(common.layer_init(layer))

        self.features_extractor_hexes = torch.nn.Sequential()
        for spec in network.features_extractor_hexes:
            layer = AgentNN.build_layer(spec)
            self.features_extractor_hexes.append(common.layer_init(layer))
    ...
    def extract_features(self, x):
        ...
        fstacks = self.features_extractor_stacks(stacks)
        fhexes = self.features_extractor_hexes(hexes)
        return torch.cat((fstacks, fhexes), dim=1)
...
def main(args, agent_cls=Agent):
    ...
    try:
        ...
        next_lstm_feat_seq = torch.zeros_like(lstm_feat_seqs[0])
        ...
        while progress < 1:
            ...
            for step in range(0, args.num_steps):
                ...
                lstm_feat_seqs[step] = next_lstm_feat_seq
            ...
            with torch.no_grad():
                next_value = agent.NN.get_value(next_lstm_feat_seq, next_lstm_hstate, next_lstm_cstate).reshape(1, -1)
            ...
            b_lstm_feat_seqs = lstm_feat_seqs.flatten(end_dim=1)
            ...
            for epoch in range(args.update_epochs):
                ...
                for start in range(0, batch_size, minibatch_size):
                    ...
                    mb_lstm_obs_seqs = b_lstm_obs_seqs[mb_inds]

                    # XXX:
                    # extract_features() expects (B, obs), however
                    # mb_lstm_obs_seqs has shape (B, seq_len, obs)
                    # => flatten, process, unflatten
                    #  (with num_mb=4, num_steps=256 => mb_size=64
                    #   => avg. 4 steps spacing b/w mb indexes
                    #   => ~25% overcalcualtion of features (seq_len=5)
                    #   Not sure if that's not desired, as caching the
                    #   features might affect gradient calculations.

                    mb_lstm_obs_seqs_flat = mb_lstm_obs_seqs.flatten(end_dim=1)
                    # => (B*N, *OBS)

                    mb_lstm_feat_seqs_flat = agent.NN.extract_features(mb_lstm_obs_seqs_flat)
                    # => (B*N, *FEAT)

                    assert mb_lstm_feat_seqs_flat.shape[0] == minibatch_size * lstm.seq_len

                    mb_lstm_feat_seqs = mb_lstm_feat_seqs_flat.reshape(minibatch_size, lstm.seq_len, -1)
                    # => (B, N, *FEAT)
