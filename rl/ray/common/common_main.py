...
def load_ray_checkpoint(path):
    ...
    with open(util.to_abspath(f"{path}/algo_config.json"), "r") as f:
...
def new_tuner(algo_config_cls, algo_cls, package_rel, opts):
    ...
    match opts.init_method:
        ...
        case "load_python_config":
            ...
            mod = importlib.import_module(opts.init_argument, package=package_rel)
    ...
    algo_config = algo_config_cls().master_config(master_config)
...
def resume_tuner(opts, algo_cls):
    ...
    return ray.tune.Tuner.restore(path=str(restore_path), trainable=algo_cls.ALGO_NAME)


def main(algo_config_cls, algo_cls, package_rel):
    algo_name = algo_cls.ALGO_NAME

    vcmi_gym.register_envs()
    ray.tune.registry.register_trainable(algo_name, algo_cls)

    parser = argparse.ArgumentParser()
    parser.add_argument("-n", "--experiment-name", type=str, metavar="<name>", default=f"{algo_name}-{{datetime}}", help="Experiment name (if starting a new experiment)")
...
Example:
  python -m rl.ray.<algo>.main ...

  ... -n "{algo_name}-test-{{datetime}}"
  ... -m load_wandb_run -a s-manolloff/newray/35091_00000
  ... -m load_wandb_artifact -a s-manolloff/newray/model.pt:v52
  ... -m resume_experiment -a data/{algo_name}-20241106_163258 -e VCMI-v4
...
"""
    ...
    if opts.init_method == "resume_experiment":
        tuner = resume_tuner(opts, algo_cls)
        checkpoint_load_dir = None
    else:
        tuner, checkpoint_load_dir = new_tuner(algo_config_cls, algo_cls, package_rel, opts)
