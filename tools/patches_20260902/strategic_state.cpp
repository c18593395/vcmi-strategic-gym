...
uint8_t* g_terrain_grid = nullptr;  // Phase I.4: pointer, allocated once: 独立全局地形栅格
}
...
static void fill_exploration(StrategicState* state, CGameState& gs) {
    ...
    // active_hero: g_active_hero 是 AAI 侧 getHeroesInfo()(仅己方 red) 的索引, 必须落在 red 英雄槽位。
    // H.4 全知直读后 heroes 段含所有玩家英雄, 若 red 英雄战死使 g_active_hero 越界到 blue 槽位,
    // 仅靠 id>=0 校验会误把 blue 英雄当 active → passable 围绕敌方英雄计算 → moveHero 全被拒。
    // 故强制 owner==0, red 无英雄时置 -1 (调用方需防护 active_hero<0)。
    int32_t ah = g_active_hero;
    if (ah < 0 || ah >= MAX_HEROES || state->heroes[ah].id < 0 || state->heroes[ah].owner != 0)
        ah = 0;
    if (ah < 0 || ah >= MAX_HEROES || state->heroes[ah].id < 0 || state->heroes[ah].owner != 0)
        ah = -1;
    ...
    // 中心用英雄可访问位置 (visitablePos = pos - getVisitableOffset()) — 与 CGameHandler::moveHero
    // 的 convertToVisitablePos 对齐: 英雄实际站立格是 visitablePos, 而 heroes[].pos 是对象锚点
    // (本仓库英雄模板 mask ["VVV","VAV"] → visitableOffset=(1,0))。若围绕锚点计算, 15×15 窗口与
    // 8 方向 passable mask 会整体偏移 offset, 导致 passable=1 的方向服务器检查到错位格而拒绝。
...
// H.8: target_list 填充 — 最近 8 个可采集目标 (MOVE_TO 动作 24)
// 候选: 未占矿 / 资源堆 / 篝火 / 宝箱 / 宝物 (同层, 曼哈顿最近, 守卫战力 log2)
static void fill_target_list(StrategicState* state, CGameState& gs) {
    std::shared_lock gsLock(CGameState::mutex);
    ...
    int n = std::min<int>(TARGET_LIST, (int)cands.size());
    for (int i = 0; i < n; i++) {
    ...
    for (int i = n; i < TARGET_LIST; i++)
        for (int j = 0; j < TARGET_DIM; j++)
            state->target_list[i][j] = 0;
}


// I.2: BFS pathfinding for MOVE_TO - compute first-step direction for each target
// Writes direction codes (0-7) to reserved[0..7], -1 = unreachable
// Diagnostics in reserved[8..15] for Python verification
static void fill_next_dir(StrategicState* state, CGameInfoCallback* gicb) {
    for (int i = 0; i < 16; i++)
        state->reserved[i] = -1;
    ...
    // 用 visitablePos (与 passable[8]/fill_exploration 同坐标系)
    ...
    for (int t = 0; t < TARGET_LIST; t++) {
        ...
        while (qh < (int)q.size() && !found) {
            ...
            for (int d = 0; d < 8 && !found; d++) {
                ...
                // Target tile: always reachable (may have visitable object that blocks)
...
// Phase I.4: 地形栅格填充 (21x21x4, uint8, HWC)
// 以英雄为中心, 采集周围 10 格的地形/障碍/对象/可见性
// 开关: 取消注释以启用地形栅格采集
#define ENABLE_TERRAIN_GRID
#ifndef ENABLE_TERRAIN_GRID
// 开关未定义时, 函数体为空 (零开销)
static void fill_terrain_grid(StrategicState* state, CGameInfoCallback* gicb) {}
    fprintf(stderr, "[TERRAIN] fill_terrain_grid called ah=%d\n", state->active_hero);
#else
static void fill_terrain_grid(StrategicState* state, CGameInfoCallback* gicb) {
        fprintf(stderr, "[TERRAIN] fill_terrain_grid called ah=%d\n", state->active_hero);
    ...
    for (int r = 0; r < GRID_SIZE; r++) {
        for (int c = 0; c < GRID_SIZE; c++) {
            ...
            // C0: terrain type (uint8, 直接存枚举值)
            state->terrain_grid[idx + 0] = (uint8_t)tile.terrainType;

            // C1: blocked
            state->terrain_grid[idx + 1] = tile.blocked() ? 1 : 0;

            // C2: object type (优先级: hero>town>monster>artifact>chest>resource>mine>dwelling>teleport>other)
            uint8_t obj_type = 0;
    ...
    // Write terrain_grid to shared file for Python reading
    {
        FILE* fp = fopen("/home/administrator/vcmi-workspace/terrain_grid.bin", "wb");
        if (fp) { fwrite(state->terrain_grid, 1, GRID_TOTAL, fp); fclose(fp); }
    }
    fprintf(stderr, "[TERRAIN] wrote terrain_grid to /home/administrator/vcmi-workspace/terrain_grid.bin\n");
}
#endif
...
static std::atomic<int> s_turn_generation{0};   // turn generation: incremented per process_turn
static int s_last_waited_gen = 0;               // generation consumed by last Python wait
...
static void fill_strategic_state(void* cbPtr) {
        ...
        if (state->active_hero >= 0 && state->heroes[state->active_hero].id >= 0) {
            ...
            // 与 CGameHandler::moveHero 对齐 (moveHero 的 dst 是锚点语义, 服务器检查
            // convertToVisitablePos(dst) = dst - getVisitableOffset()): 英雄站立格 = heroes[].pos 的
            // visitablePos, 目标格 = 站立格 + dir == 服务器检查的 convertToVisitablePos(pos+dir)。
            // 若用锚点 pos+dir 作为目标格, 会偏移 hero visitableOffset (3x2 英雄模板 offset=(1,0)),
            // 造成 passable=1 的方向被服务器以 blocked 拒绝。
                ...
                // Towns — H.4 全知直读 (8-2 fill 遗漏, 补上; 遍历 gs.players 拿全部城镇)
        {
            ...
            for (const auto& [color, ps] : gs3.players) {
                ...
                for (const auto* town : ps.getTowns()) {
                    ...
                    // 2026-09-01 P3 取兵链路: 驻军填充 — getUpperArmy (garrisonHero private,
                    // 同 is_garrisoned 技巧): 敌城=守军, 无 visiting 时=城防兵力;
                    // obs 城槽 field 6-12 已预留, Python 侧无需改动
                    {
        ...
        // H.8: target_list (MOVE_TO 目标列表)
        fill_target_list(state, gicb->gameState());

        // I.2: next_dir BFS
        fill_next_dir(state, gicb);

        // H.5: v3 扩展字段 (战力/驻守/招募/建造/威胁/预测)
        fill_v3_fields(state, gicb);

        // Phase I.4: 地形栅格 (ENABLE_TERRAIN_GRID 开关控制)
        fill_terrain_grid(state, gicb);
}
...
extern "C" void adventure_process_turn(int playerColor, void* userData) {
    ...
    // clear ready BEFORE fill: prevent stale true (from MMAI send_action(-1)) racing the wait loop
    ...
    s_turn_generation.fetch_add(1, std::memory_order_acq_rel);  // new turn generation
...
extern "C" int adventure_get_action() {
    int a = s_turn_action.load(std::memory_order_acquire);
    return a;
}
...
extern "C" void strategic_state_update(void* game_state_ptr) {
    ...
    if (!game_state_ptr) return;
    {
        ...
        if (state.active_hero >= 0 && state.heroes[state.active_hero].id >= 0) {
            ...
            static const int32_t dx[8] = {0,1,1,1,0,-1,-1,-1};
            static const int32_t dy[8] = {-1,-1,0,1,1,1,0,-1};
            // 与 CGameHandler::moveHero 的 convertToVisitablePos 对齐: 目标格 = 站立格(visitablePos) + dir,
            // 而非锚点 pos + dir (英雄模板 visitableOffset=(1,0), 锚点语义会整体错位一格)
            const CGHeroInstance* hero = dynamic_cast<const CGHeroInstance*>(gs.getObjInstance(ObjectInstanceID(state.heroes[state.active_hero].id)));
            int3 hpos(state.heroes[state.active_hero].pos_x, state.heroes[state.active_hero].pos_y, state.heroes[state.active_hero].pos_z);
            if (hero) hpos = hero->visitablePos();
            auto& hpos_tile = gs.getMap().getTile(hpos);
            for (int d = 0; d < 8; d++) {
                int3 target(hpos.x + dx[d], hpos.y + dy[d], hpos.z);
