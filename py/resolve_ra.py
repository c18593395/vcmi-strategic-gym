...
for target in (0x37AA4F, 0x1C36C9, 0x2AB991):
    fr = fn_of(target)
    if not fr:
        print('=== 0x%x: NO FUNCTION ===' % target)
        continue
    fs, fe = fr
    print('=== ra 0x%x in fn [0x%x - 0x%x) size=0x%x (offset in fn: 0x%x) ===' % (target, fs, fe, fe - fs, target - fs))
    o = rva2off(fs)
    pe.seek(o)
    code = pe.read(min(fe - fs, 0x400))
    count = 0
    for ins in md.disasm(code, imgbase + fs):
        ...
        count += 1
        if count > 90 or (ins.mnemonic == 'ret' and rva >= target):
            break
    print()
