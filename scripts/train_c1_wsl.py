#!/usr/bin/env python3
"""C1 PPO Training — WSL-native, subprocess-per-episode within WSL"""
...
MODEL = "/mnt/d/Bigdata/hero3_fresh/c1_model.pt"
...
def run_episode(turns):
    """Spawn subprocess for one episode, return {steps, rew}"""
    env = os.environ.copy()
    ...
    try:
        r = subprocess.run(
            [sys.executable, "-u", RUNNER, str(turns), TRAJ_PKL],
            capture_output=True, timeout=20, env=env
        )
    except subprocess.TimeoutExpired:
        pass  # pickle+json already written
