...
if (os.getenv("VCMIGYM_DEBUG", None) == "1"):
    from ...connectors.build import exporter_v15 as exporter
else:
...
TRACE = os.getenv("VCMIGYM_DEBUG", None) == "1"
MAXLEN = 80
...
class PyConnector():
    ...
    def start_connector(self, **kwargs):
        ...
        if (os.getenv("VCMIGYM_DEBUG", None) == "1"):
            ...
            from ...connectors.build import connector_v15
        else:
            from ...connectors.rel import connector_v15

        self.logger.debug("VCMI connector kwargs: %s" % str(kwargs))
        self._connector = connector_v15.ThreadConnector(
            maxlogs=self.maxlogs,
            bootTimeout=self.boot_timeout,
            vcmiTimeout=self.vcmi_timeout,
            userTimeout=self.user_timeout,
            **kwargs
        )
